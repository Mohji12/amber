#!/usr/bin/env python3
"""
Bulk Update S3 Images Script
This script fetches S3 URLs programmatically and uses batch UPDATE statements for fast bulk updates.
"""
...
def get_s3_urls(bucket_name, prefix="Amber/New+folder/"):
    ...
    try:
        ...
        print("💡 Make sure you have AWS credentials configured or provide them manually")
        return {}

def bulk_update_subcategories(engine, s3_urls):
    ...
    try:
        with engine.connect() as connection:
            ...
            for subcategory in subcategories:
                ...
                if subcategory_name in s3_urls:
                    updates.append((subcategory.id, s3_urls[subcategory_name]))
                    matched_count += 1
                    continue
                ...
                if cleaned_name in s3_urls:
                    updates.append((subcategory.id, s3_urls[cleaned_name]))
                    matched_count += 1
                    continue
...
def main():
    ...
    print("🚀 Starting Bulk S3 Image URL Update...")
    
    # Configuration
    BUCKET_NAME = "jgi-menteetracker"  # Your S3 bucket name
    S3_PREFIX = "Amber/New folder/"    # Your S3 folder prefix
    ...
    # Update subcategories
    subcategories_success = bulk_update_subcategories(engine, s3_urls)
