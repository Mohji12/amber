...
This script completely replaces all product image URLs with the correct Amber/products/ path.
"""
...
def replace_all_product_images(engine, s3_urls):
    """Replace all product image URLs with correct S3 path"""
    print("\n🔄 Replacing all product image URLs...")
    ...
    try:
        with engine.connect() as connection:
            ...
            if updates:
                ...
                print("🧹 Clearing all existing image URLs...")
                connection.execute(text("UPDATE products SET image_url = NULL"))
                
                # Then set the new ones
                print("🔄 Setting new image URLs...")
                update_sql = """
                    UPDATE products 
...
def main():
    """Main function to fix all product images"""
    print("🚀 Starting Complete Product Image Fix...")
    ...
    products_success = replace_all_product_images(engine, s3_urls)
    
    if products_success:
        print("\n🎉 Complete product image fix completed successfully!")
        return True
    else:
        print("\n💥 Product image fix failed. Please check the errors above.")
        return False
